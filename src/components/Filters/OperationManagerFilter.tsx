...
const OperationManagerFilter = () => {
  ...
  const [OmNames, setOmNames] = useState([])
  const { resetOMNames, setResetOMNames } = useMenuStore()
  const { addOmsNames } = useFiltersStore()
  useEffect(() => {
    if (data) {
      const OMList = data
        .map((name: any) => {
          const agentsName = name['operations_manager_name']
          return agentsName
        })
        .sort()
      setFilteredData(OMList)
      setOmNames(OMList)
    }

    if (resetOMNames) {
      setSelectedNames([])
      setResetOMNames(false)
    }
  }, [data, resetOMNames, setResetOMNames])
  ...
  const handleAlphabetClick = (alphabet: any) => {
    ...
    const filteredNames = OmNames.filter((item: any) => {
  ...
  const handleSearchChange = () => {
    ...
    const filteredNames = OmNames.filter(
