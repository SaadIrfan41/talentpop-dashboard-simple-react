...
const getLowActivityReportInternalTeam = async (
  ...
  try {
    const res = await fetch(
      `http://18.237.25.116:8000/low-activity-rate-report-internal-team-percentage?${clientQueryParam}&${agentsQueryParam}&${teamLeadQueryParam}&${OM_QueryParam}&${CSM_QueryParam}&startdate=${startingDateFilter}&enddate=${endingDateFilter}`,
      {
        headers: {
...
const LowActivityReportInternalTeam = () => {
  const {
    filterClientName,
    filterAgentsName,
    filterCSMsName,
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      getLowActivityReportInternalTeam(
        filterClientName,
        filterAgentsName,
        filterTeamLeadsName,
        filterOMsName,
...
export default LowActivityReportInternalTeam
