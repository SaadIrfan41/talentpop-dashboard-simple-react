...
const getInternalTeamHighActivityReport = async (
  ...
  try {
    const res = await fetch(
      `http://18.237.25.116:8000/high-activity-rate-report-internal-team?${clientQueryParam}&${agentsQueryParam}&${teamLeadQueryParam}&${OM_QueryParam}&${CSM_QueryParam}&startdate=${startingDateFilter}&enddate=${endingDateFilter}`,
      {
        headers: {
...
const InternalTeamHighActivityReport = () => {
  const {
    filterClientName,
    filterAgentsName,
    filterCSMsName,
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      getInternalTeamHighActivityReport(
        filterClientName,
        filterAgentsName,
        filterTeamLeadsName,
        filterOMsName,
  ...
  data?.forEach((obj: any) => {
    ...
    activityAvg.push(obj['avg_activity'].toFixed(2).replace(/[.,]00$/, ''))
  })
...
export default InternalTeamHighActivityReport
